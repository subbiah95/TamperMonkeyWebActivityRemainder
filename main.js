...
(function() {
    ...
    function updateTimerData() {
        ...
        if (data) {
            // Add time elapsed since last update to total time
            data.totalTime += (now - data.lastUpdate);
            data.lastUpdate = now;
            stored[domain] = data;
            saveTimers(stored);
        }
    ...
    function getCurrentElapsedTime() {
        ...
        return data.totalTime + sessionTime;
    }
    ...
    document.addEventListener('visibilitychange', function() {
        ...
        if (isVisible && !wasVisible) {
            ...
            let stored = getStoredTimers();
            if (stored[domain]) {
                stored[domain].sessionStart = Date.now();
                stored[domain].lastUpdate = Date.now();
                saveTimers(stored);
            }
        } else if (!isVisible && wasVisible) {
            // Tab became hidden - update total time
            updateTimerData();
        }
    ...
    window.addEventListener('focus', function() {
        ...
        timerData = getStoredTimers()[domain] || timerData;
    });
